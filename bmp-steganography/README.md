...
```
## ✅ Expected Output
Upon successful execution, the program will generate a new valid image file named `stego.bmp` in the active directory and output the successfully decrypted payload to the terminal:
```bash
The quick brown fox jumps over the lazy dog
```
### 🖼️ Visual Output
Here is the actual 32x32 bitmap generated by the program, containing the XOR-encrypted payload hidden within the RGB pattern:

![Generated Steganography Bitmap](stego.bmp)
